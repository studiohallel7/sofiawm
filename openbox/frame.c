...
static void layout_title(ObFrame *self){
    ...
    /* 1. Fechar — posição correta, background None para não cobrir Cairo */
    self->close_x = x;
    ...
    XSetWindowBackgroundPixmap(obt_display, self->close, None);
    ...
    XSetWindowBackgroundPixmap(obt_display, self->iconify, None);
    x += btn_width + espacamento;
    ...
    XSetWindowBackgroundPixmap(obt_display, self->max, None);
    x += btn_width + espacamento;
